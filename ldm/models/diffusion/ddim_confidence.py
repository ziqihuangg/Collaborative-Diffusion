"""multi-modal-driven DDIM sampling using dynamic diffusers and uni-modal pre-trained diffusion models"""
...
class DDIMConfidenceSampler(object):
    def __init__(self, model, return_confidence_map=False, seg_mask_scale_factor=2.0, seg_mask_schedule = 'cosine_decay',softmax_twice=False, boost_factor=1.0, manual_prob=1.0, return_each_branch=False, schedule="linear", conditions= ['seg_mask', 'text'], **kwargs):
        ...
        self.model.model.return_confidence_map = return_confidence_map

        self.conditions = conditions
        self.model.model.seg_mask_scale_factor = seg_mask_scale_factor
        self.model.model.seg_mask_schedule = seg_mask_schedule
        self.model.model.softmax_twice = softmax_twice
        self.model.model.boost_factor = boost_factor
        self.model.model.manual_prob = manual_prob
        self.model.model.return_each_branch = return_each_branch
        self.return_each_branch = return_each_branch

        if return_each_branch:
            assert return_confidence_map
    ...
    def p_sample_ddim(self, x, c, t, index, repeat_noise=False, use_original_steps=False, quantize_denoised=False,
        ...
        if unconditional_conditioning is None or unconditional_guidance_scale == 1.:
            if self.return_confidence_map:
                outputs = self.model.apply_model(x, t, c)
                e_t = outputs['output']
